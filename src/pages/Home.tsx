      ...
      {isModal ? (
        ...
        <div className="overscroll-none overflow-x-hidden bg-emerald-950">
          <div className="flex flex-col z-0 relative w-screen h-screen bg-emerald-950">
            ...
            <div className="xl:mt-32 mt-28">
              <p className="xl:text-4xl text-3xl text-center w-screen font-semibold text-green-500">Arquivos do agente</p>
              {isTyping && (
                showTypingEffect.effect1 ? (
                  <TypingEffect
                    ...
                    texts={texts}
                    textClass="p-4 xl:pl-48 pl-20 mt-6 text-green-500 xl:text-3xl text-2xl font-semibold"
                    divClass="xl:mt-6 mt-1"
                  />
                ) : (
                  <div className = "flex flex-col mt-6 p-5">
                  {texts.map((text, i) => (
                    <p key={i} className="p-4 xl:pl-48 pl-20 xl:mt-6 mt-1 text-green-500 xl:text-3xl text-2xl font-semibold">{text}</p>
                  ))}
                  </div>
                )
              )}
            ...
            <div className="absolute top-0 left-0 flex w-screen h-screen justify-center items-center z-0">
              <img src="images/na verde com texto.svg" className=" mt-64 xl:h-96 h-80" />
            </div>
          </div>
          <div className="h-full bg-emerald-950">
            <div className="xl:h-2 h-1"></div>
            <div ref={el => sectionsRef.current[0] = el} className="z-1 h-fit">
              {type[0] ? (showTypingEffect.effect2 ? (
                <TypingEffect
                  ...
                  speed={30}
                  textClass="leading-relaxed text-justify p-20 xl:px-48 px-20 mt-6 text-green-500 xl:text-2xl texl-xl font-semibold"
                  divClass="mt-6"
                />
              ) : (
                corpoText.map((text, i) => (
                  <p key={i} className="leading-relaxed text-justify p-20 xl:px-48 px-20 mt-6 text-green-500 xl:text-2xl text-xl font-semibold">{text}</p>
                ))
              )) : null}
            </div>
            ...
            <div ref={el => sectionsRef.current[1] = el} className="h-fit">
              {type[1] ? (showTypingEffect.effect2 ? (
                <TypingEffect
                  ...
                  texts={corpoText2}
                  speed={30}
                  textClass="leading-relaxed text-justify  xl:px-48 px-20 my-6 text-green-500 xl:text-2xl text-xl font-semibold"
                  divClass="mt-6"
                />
              ) : (
                corpoText2.map((text, i) => (
                  <p key={i} className="leading-relaxed text-justify xl:px-48 px-20 my-6 text-green-500 xl:text-2xl text-xl font-semibold">{text}</p>
                ))
              )) : null}
            </div>
